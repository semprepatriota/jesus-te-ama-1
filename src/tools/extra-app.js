...
let controls = '';
if (operation === 'cut') controls = `<div class="range-row"><div><label class="field-label" for="start">In\u00edcio (segundos)</label><input id="start" type="number" min="0" step="0.01" value="0"><button class="point-button" id="set-start">${icon('play')}Usar instante atual</button></div><div><label class="field-label" for="end">Fim (segundos)</label><input id="end" type="number" min="0.2" step="0.01" value="1"><button class="point-button" id="set-end">${icon('play')}Usar instante atual</button></div></div><input id="seek" type="range" min="0" max="1" step="0.01" value="0" aria-label="Instante da previa"><div class="trim-duration" id="range-duration"></div><p class="compatibility">Corte exato com recompressao, inclusive entre keyframes. Precisao limitada ao quadro e aos blocos de audio; a qualidade pode mudar.</p>`;
if (operation === 'clean') controls = `<p class="step-label">METADADOS DESCRITIVOS</p><label class="all-fields"><input id="all-fields" type="checkbox" disabled>Selecionar todos os campos encontrados</label><div class="metadata-list" id="fields"><p class="metadata-empty">Nenhum video inspecionado.</p></div><p class="compatibility">MP4/MOV H.264/AAC, sem recodificacao. Rotacao, cor e parametros de reproducao permanecem. Se um campo nao escolhido mudar, a copia sera bloqueada.</p>`;
...
let engine, loading, metadata, file, previewUrl, result, busy = false, generation = 0;
const history = [];
...
function buttons() {
  ...
  if (operation === 'audio') supported &&= !!metadata.audio?.decodable;
...
async function open(next) {
  ...
  try {
    ...
    if (operation === 'clean') {
      ...
      for (const field of data.fields) { const label = document.createElement('label'), input = document.createElement('input'), span = document.createElement('span'), strong = document.createElement('strong'), small = document.createElement('small'); input.type = 'checkbox'; input.dataset.field = field.id; strong.textContent = field.label; small.textContent = field.value; span.append(strong,small); label.append(input,span); $('#fields').append(label); }
    }
    status('V\u00eddeo analisado', operation === 'audio' && !data.audio ? 'Este video nao possui audio.' : operation === 'clean' ? 'Escolha os campos que deseja remover. A copia sera reinspecionada.' : 'Pronto para gerar uma copia local.');
...
async function run() {
  ...
  try {
    const options = operation === 'cut' ? { start: Number($('#start').value), end: Number($('#end').value) } : operation === 'clean' ? { selected: selection() } : operation === 'audio' ? { format: $('#format').value } : { timestamp: Number($('#timestamp').value), format: $('#format').value };
