...
const before = verifyPublic(directory, { publicPreview });
const build = spawnSync(process.execPath, ['scripts/build-public.mjs', ...(publicPreview ? ['--public-preview'] : [])], { encoding: 'utf8' });
if (build.status !== 0) throw new Error(build.stdout + build.stderr);
const after = verifyPublic(directory, { publicPreview });
