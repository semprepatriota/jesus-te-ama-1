...
console.log(JSON.stringify({ mode: production ? 'production-awaiting-release-gate' : 'preview-not-publishable', directory: 'dist', files: allowed.length, pages: production ? 27 : 26, indexedPages: production ? routes.filter(route => route.indexable).length : 0, published: false }));
