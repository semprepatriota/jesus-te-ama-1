...
for (const route of routes.filter(route => production || publicPreview || route.group !== 'error')) write(route.file, prepareHtml(fs.readFileSync(route.file, 'utf8'), route, { origin: rules.canonicalOrigin, production }));
...
write('robots.txt', robots(rules.canonicalOrigin, production));
write('sitemap.xml', sitemap(routes, rules.canonicalOrigin, production));
console.log(JSON.stringify({ mode: production ? 'production-awaiting-release-gate' : publicPreview ? 'public-preview' : 'preview-not-publishable', directory, files: allowed.length, pages: production || publicPreview ? 27 : 26, indexedPages: production ? routes.filter(route => route.indexable).length : 0, published: false }));
